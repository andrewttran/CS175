...
static shared_ptr<SgRbtNode> g_currentPickedNode;
...
static shared_ptr<Geometry> g_ground, g_cube, g_sphere, g_sphere2;

// --------- Scene

static const Cvec3 g_light1(2.0, 3.0, 14.0), g_light2(-2, -3.0, -5.0);  // define two lights positions in world space
static RigTForm frames[3];
static shared_ptr<SgRbtNode> frameNodes[3];
...
static void initSpheres() {
	...
	makeSphere(1, slices, stack, vtx.begin(), idx.begin());
	g_sphere2.reset(new Geometry(&vtx[0], &idx[0], vbLen, ibLen));
...
static void drawStuff(const ShaderState& curSS, bool picking) {
  ...
  const RigTForm eyeRbt = getPathAccumRbt(g_world, frameNodes[g_activeCamera]);
  ...
  if (!(g_mouseMClickButton || (g_mouseLClickButton && g_mouseRClickButton) || (g_mouseLClickButton && !g_mouseRClickButton && g_spaceDown) )) {
	  if (g_currentPickedRbtNode != NULL)
	    g_arcballScale = getScreenToEyeScale(((invEyeRbt * (*g_currentPickedRbtNode).getRbt()).getTranslation())[2], g_frustFovY, g_windowHeight);
	  else
		g_arcballScale = getScreenToEyeScale(((invEyeRbt * (*g_skyNode).getRbt()).getTranslation())[2], g_frustFovY, g_windowHeight);
  }
  else {
	 g_arcballScale = 0.01;
  }
    ...
    RigTForm temp = transFact(getPathAccumRbt(g_world, g_currentPickedRbtNode));
    if (g_currentPickedRbtNode == g_skyNode) {
	  temp = RigTForm();
      g_arcballScale = 0.01;}

    Matrix4 MVM = rigTFormToMatrix(invEyeRbt * temp) * Matrix4::makeScale(Cvec3(g_arcballScale, g_arcballScale, g_arcballScale));
  ...
  else { // if picking is true
    Picker picker(invEyeRbt, curSS);
    g_world->accept(picker);
    glFlush();
    g_currentPickedRbtNode = picker.getRbtNodeAtXY(g_mouseClickX, g_mouseClickY);
    if (g_currentPickedRbtNode == g_groundNode)
      g_currentPickedRbtNode = shared_ptr<SgRbtNode>();   // set to NULL - note this means anything that isn't a robot is set to NULL, i.e. sky too
  }
...
static void motion(const int x, const int y) {
  ...
  if (g_mouseLClickButton && !g_mouseRClickButton && !g_spaceDown) { // left button down?
	...
	// getScreenSpaceCoord to find center of sphere - not sure how to feed it eye coordinates? // got rid of 1 here
	Cvec2 center = getScreenSpaceCoord((inv(frames[g_activeCamera]) * getPathAccumRbt(g_world, g_currentPickedRbtNode)).getTranslation(), 
    ...
    m = RigTForm(Cvec3(dx * 0.01, dy * g_arcballScale, 0));
  }
  ...
  if (g_mouseClickDown) {
	RigTForm accum_node = RigTForm();
	RigTForm accum_prev = RigTForm();
	// prevent it from updating
	// store the shoulder coordinates

	accum_node = getPathAccumRbt(g_world, g_currentPickedRbtNode);
		
	accum_prev = getPathAccumRbt(g_world, g_currentPickedRbtNode, 1);
	//accum_node = transFact(frames[g_activeObject]) * linFact(frames[g_activeCamera]);
	
	RigTForm accum_eye = getPathAccumRbt(g_world, frameNodes[g_activeCamera]);
	

	// make sure it has the nodes center and the eye's direction
	a = transFact(accum_node) * linFact(accum_eye);
	
	// find the transformation with respect to the parent instead of the world
	RigTForm as = inv(accum_prev) * a;

	//doMtoOwrtA(M, L, as) calculation
	(*g_currentPickedRbtNode).setRbt(as * m * inv(as) * (*g_currentPickedRbtNode).getRbt());
	//frames[g_activeObject] = a * m * inv(a) * frames[g_activeObject];
...
static void constructRobot(shared_ptr<SgTransformNode> base, const Cvec3& color) {
	...
	ShapeDesc shapeDesc[NUM_SHAPES] = {
		...
		{ 1, ARM_LEN / 2, 0, 0, ARM_LEN / 2, ARM_THICK / 2, ARM_THICK / 2, g_sphere2 }, // upper right arm
		{ 2, ARM_LEN / 2, 0, 0, ARM_LEN, ARM_THICK, ARM_THICK, g_cube }, // lower right arm
		{ 3, -ARM_LEN / 2, 0, 0, -ARM_LEN / 2, ARM_THICK / 2, ARM_THICK / 2, g_sphere2 }, // upper left arm
		{ 4, -ARM_LEN / 2, 0, 0, -ARM_LEN, ARM_THICK, ARM_THICK, g_cube }, // lower left arm
		{ 5, 0, -ARM_LEN/2, 0, ARM_THICK / 2, -ARM_LEN / 2, ARM_THICK / 2, g_sphere2 }, // upper right leg
		{ 6, 0, -ARM_LEN/2, 0, ARM_THICK, -ARM_LEN, ARM_THICK, g_cube }, // lower right leg	
		{ 7, 0, -ARM_LEN/2, 0, ARM_THICK / 2, -ARM_LEN / 2, ARM_THICK / 2, g_sphere2 }, // upper left leg
		{ 8, 0, -ARM_LEN/2, 0, ARM_THICK, -ARM_LEN, ARM_THICK, g_cube }, // lower left leg
		{ 9, 0, TORSO_WIDTH / 2, 0, TORSO_WIDTH / 2, TORSO_WIDTH / 2, TORSO_THICK / 2, g_sphere2 } // head
	};
...
static void initScene() {
  ...
  frames[0] = (*g_skyNode).getRbt();
  frames[1] = (*g_robot1Node).getRbt();
  frames[2] = (*g_robot2Node).getRbt();

  frameNodes[0] = g_skyNode;
  frameNodes[1] = g_robot1Node;
  frameNodes[2] = g_robot2Node;

  g_currentPickedRbtNode = g_skyNode;

  cout << "g_world: " << g_world << "\n"
	  << "g_skyNode: " << g_skyNode << "\n"
	  << "g_ground: " << g_ground << "\n"
	  << "g_robot1Node: " << g_robot1Node << "\n"
	  << "g_robot2Node: " << g_robot2Node << "\n";
