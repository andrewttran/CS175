...
bool Picker::visit(SgTransformNode& node) {
  ...
  // push (*q).getRbt() * node.getRbt() onto nodeStack_?
...
bool Picker::visit(SgShapeNode& node) {
  // TODO
  idCounter_ += 1;
  shared_ptr<SgRbtNode> joint = dynamic_pointer_cast<SgRbtNode>(nodeStack_.back());
  addToMap(idCounter_, joint);
  // convert ID to color and pass to shader
