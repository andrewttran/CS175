...
static Matrix4 frames[3] = {g_skyRbt, g_objectRbt[0], g_objectRbt[1]}; // use this to manipulate & switch views
...
static void drawStuff() {
  ...
  const Matrix4 eyeRbt = frames[g_activeCamera];
  ...
  // draw cubes
  // ==========
  MVM = invEyeRbt * frames[1];
  NMVM = normalMatrix(MVM);
  sendModelViewNormalMatrix(curSS, MVM, NMVM);
  safe_glUniform3f(curSS.h_uColor, g_objectColors[0][0], g_objectColors[0][1], g_objectColors[0][2]);
  g_cube->draw(curSS);

  MVM = invEyeRbt * frames[2];
  NMVM = normalMatrix(MVM);
  sendModelViewNormalMatrix(curSS, MVM, NMVM);
