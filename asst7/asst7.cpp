...
static bool g_shading = true; // if using smooth shading, set to true
...
static int g_subdivision = 0;
static float g_cubeSpeed = 1.0;
...
static shared_ptr<Mesh> g_objectMesh, g_cubeMesh;
...
static shared_ptr<SimpleIndexedGeometryPN> g_object;
...
static shared_ptr<SgRbtNode> g_skyNode, g_groundNode, /*g_robot1Node, g_robot2Node,*/ g_light1Node, g_light2Node, g_objectNode;
...
// write a function to average the normals
static Mesh average_normals(Mesh mesh) {
	// loop through all the vertices
	for (int i = 0; i < mesh.getNumVertices(); i++) {
		Mesh::Vertex v = mesh.getVertex(i);
		// sum the normals of all the faces incident ot the vertex in the vertex
		Mesh::VertexIterator it(v.getIterator()), it0(it);
		Cvec3 sum;
		// find the sum of the normals
		do
		{
			Cvec3 normal = (it.getFace()).getNormal();
			sum = sum + normal;

		} while (++it != it0); // go around once the 1ring

		v.setNormal(normalize(sum));
	}
	return mesh;
}

static void createMeshToSimple(Mesh mesh, vector<VertexPN>::iterator vtxIter, vector<unsigned int>::iterator idxIter) {
	for (int i = 0; i < mesh.getNumFaces(); i++) {
		Mesh::Face f = mesh.getFace(i);
		assert(f.getNumVertices() == 4);
		//Cvec3 nor = f.getNormal();

		if (g_shading) {
			*vtxIter = VertexPN(f.getVertex(0).getPosition(), (f.getVertex(0)).getNormal());
			*(++vtxIter) = VertexPN(f.getVertex(1).getPosition(), (f.getVertex(1)).getNormal());
			*(++vtxIter) = VertexPN(f.getVertex(2).getPosition(), (f.getVertex(2)).getNormal());
			*(++vtxIter) = VertexPN(f.getVertex(3).getPosition(), (f.getVertex(3)).getNormal());
		}
		else {
			*vtxIter = VertexPN(f.getVertex(0).getPosition(), f.getNormal());
			*(++vtxIter) = VertexPN(f.getVertex(1).getPosition(), f.getNormal());
			*(++vtxIter) = VertexPN(f.getVertex(2).getPosition(), f.getNormal());
			*(++vtxIter) = VertexPN(f.getVertex(3).getPosition(), f.getNormal());
		}
...
static void initCubeMesh() {
	...
	g_cubeMesh.reset(new Mesh(cube));
	g_objectMesh.reset(new Mesh(cube));
	cout << "cube mesh loaded successfully." << endl;

	int ibLen = 6 * cube.getNumFaces(), vbLen = 4 * cube.getNumFaces();

	// Temporary storage for cube Geometry
	vector<VertexPN> vtx(vbLen);
	vector<unsigned int> idx(ibLen);

	createMeshToSimple(average_normals(cube), vtx.begin(), idx.begin());
	g_object.reset(new SimpleIndexedGeometryPN(&vtx[0], &idx[0], vbLen, ibLen));
}
...
// write a function for subdivision
static Mesh subdivision(Mesh mesh) {
	// loop over all the faces of the mesh, compute face values, setNewFaceVertex
	for (int i = 0; i < mesh.getNumFaces(); ++i) {
		// save the the current face in f
		const Mesh::Face f = mesh.getFace(i);

		// get the number of vertices surrounding the face
		int n = f.getNumVertices();

		// find the sum of the vertices surrounding the face
		Cvec3 sum;
		for (int i = 0; i < n; ++i)
		{
			Cvec3 vertex = f.getVertex(i).getPosition();
			sum = sum + vertex;
		}

		// calculate the position
		const Cvec3& p = sum * (1. / n);
		// set the new position
		mesh.setNewFaceVertex(f, p);
	}


	// loop over all of the edges of the mesh, compute edge values, setNewEdgeVertex
	for (int i = 0; i < mesh.getNumEdges(); ++i) {
		const Mesh::Edge e = mesh.getEdge(i);

		// Calculate the sum of the surrounding vertices
		Cvec3 sum = e.getVertex(0).getPosition() + e.getVertex(1).getPosition() + mesh.getNewFaceVertex(e.getFace(0)) + mesh.getNewFaceVertex(e.getFace(1));

		// calculate the new position
		const Cvec3& p = sum * (1. / 4);
		//const Cvec3 p = normalize(p);
		// set the new edge vertex
		mesh.setNewEdgeVertex(e, p);

	}

	// loop over all vertices, compute vertex values, setNewFaceVertex
	for (int i = 0; i < mesh.getNumVertices(); ++i) {

		// get vertex and position
		const Mesh::Vertex v = mesh.getVertex(i);
		Cvec3 position = v.getPosition();

		// at each vertex
		Mesh::VertexIterator it(v.getIterator()), it0(it);

		// variable for the sum of the vertex vertices and face vertices 
		Cvec3 sum_vertex;
		Cvec3 sum_face;

		// assume valence 4 for now
		double valence = 0;
		do
		{
			// get the current vertex and its position
			Mesh::Vertex v_current = it.getVertex();
			Cvec3 position_vertex = v_current.getPosition();

			// get the current face and its position
			Mesh::Face f = it.getFace();
			Cvec3 position_face = mesh.getNewFaceVertex(f);

			// variable for the sum of the face vertices and vertex vertices
			sum_vertex = sum_vertex + position_vertex;
			sum_face = sum_face + position_face;
		...
		// calculate the position
		const Cvec3& p = position * ((valence - 2.) / valence) + sum_vertex*(1. / (valence * valence)) + sum_face*(1. / (valence * valence));
		// set the new position
	...
	mesh.subdivide();
	return mesh;
	// call my function subdivide multiple times, 3 or 4 times
	// do an optimization make OPT = 1

}


// Given t in the range [0, n], animate the corners moving
static void vertexUpdating(float t) {
	Mesh temp = *g_cubeMesh;
	for (int i = 0; i < temp.getNumVertices(); i++) {
		float s = (i % 4) / 4.;
		float scale = (sin(atan(1.0) * 4 + t*pow(2, g_cubeSpeed - 1)) * s) / 2 + 1;
		Cvec3 tempPos = (temp.getVertex(i)).getPosition();
		temp.getVertex(i).setPosition(tempPos * scale);
	}
	for (int i = 0; i < g_subdivision; i++) {
		temp = subdivision(temp);
	}

	int vbLen = temp.getNumFaces() * 4;
	int ibLen = temp.getNumFaces() * 6;

	vector<VertexPN> vtx(vbLen);
	vector<unsigned int> idx(ibLen);

	createMeshToSimple(average_normals(temp), vtx.begin(), idx.begin());
	(*g_object).upload(&vtx[0], &idx[0], vbLen, ibLen);
...
// Interpret "ms" as milliseconds into the animation
static void animateCubeMesh(int ms) {
	float t = (float)ms / (float)g_msBetweenKeyFrames;
	vertexUpdating(t);
	glutTimerFunc(50, animateCubeMesh, ms + 50);
}
...
static void keyboard(const unsigned char key, const int x, const int y) {
	...
	case '-':
		g_msBetweenKeyFrames += 100;
		break;
		// increase subdivision steps, cap at 6
	case '0':
		// cap subdivision at 5
		if (g_subdivision < 6) {
			g_subdivision++;
			Mesh temp = *g_objectMesh;
			for (int i = 0; i < g_subdivision; i++) {
				temp = subdivision(temp);
			}
			...
			cout << "subdivision " << g_subdivision << endl;
		}
		break;
	case '9':
		// cap subdivision at 0
		if (g_subdivision > 0) {
			g_subdivision--;
			Mesh temp = *g_objectMesh;
			for (int i = 0; i < g_subdivision; i++) {
...
static void initMaterials() {
	...
	g_specularMat->getUniforms().put("uColor", Cvec3f(1, 0.3, 0.9));
...
static void initScene() {
	...
	g_objectNode.reset(new SgRbtNode(RigTForm(Cvec3(0, 0, 0))));
	g_objectNode->addChild(shared_ptr<MyShapeNode>(
		new MyShapeNode(g_object, g_specularMat, (0, 0, 0))));
