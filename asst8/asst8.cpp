...
// New Geometry for the bunny
static const int g_numShells = 24; // constants defining how many layers of shells
static double g_furHeight = 0.101;
static double g_hairyness = 10.21;
...
// our implementation is pretty inefficient, check that?
static void updateShellGeometry() {
	...
	// update each shell geometry 
	for (int i = 0; i < g_numShells; ++i) {
		for (int j = 0; j < g_bunnyMesh.getNumFaces(); ++j) {
			Mesh::Face f = g_bunnyMesh.getFace(j);

			// calculate the smooth normal as nor
			//Cvec3 n1 = f.getVertex(0).getNormal(); Cvec3 n2 = f.getVertex(1).getNormal(); Cvec3 n3 = f.getVertex(2).getNormal();
			//Cvec3 nor = (n1 + n2 + n3) / 3;
			...
			for (int k = 0; k < f.getNumVertices(); ++k) {
				...
				if (k == 0) {
					texture = (0, g_hairyness);
				}
				else if (k == 1) {
					texture = (g_hairyness, 0);
				}
				else {
					texture = (0, 0);
				}

					// we need to find s in world coordinates
...
// implementation of updateShellGeometryProto prior to task 3
static void updateShellGeometryProto() {
	...
	for (int i = 0; i < g_numShells; ++i) {
		for (int j = 0; j < g_bunnyMesh.getNumFaces(); ++j) {
			...
			for (int k = 0; k < f.getNumVertices(); ++k) {
				...
				Cvec3 p = f.getVertex(k).getPosition();
				//Cvec3 s = p + (nor * g_furHeight);
				Cvec3 s = g_tipPos[f.getVertex(k).getIndex()]; //Cvec3(worldToBunny * Cvec4(g_tipPos[f.getVertex(k).getIndex()], 0));
				// the normal we calculate
				...
				if (k == 0) {
					...
					// the position we calculate
				Cvec3 position = p + n*i;
				verts.push_back(VertexPNX(position[0], position[1], position[2], n[0], n[1], n[2], texture[0], texture[1]));
				}
		}
	g_bunnyShellGeometries[i] -> upload(&verts[0], verts.size());
	verts.clear();
...
static void constructRobot(shared_ptr<SgTransformNode> base, shared_ptr<Material> material) {
	...
	JointDesc jointDesc[NUM_JOINTS] = {
		{ -1 }, // torso
		{ 0, TORSO_WIDTH / 2, TORSO_LEN / 2, 0 }, // upper right armin
...
static void initScene() {
	...
	g_meshNode.reset(new SgRbtNode(RigTForm(Cvec3(0, 1, -8))));
...
static void hairsSimulationCallback(int dontCare) {
	// something about performing multiple steps of updates with respect to g_numStepsPerFrame
	...
	// REMEMBER TO DO CALCULATIONS IN WORLD COORDS
	for (int i = 0; i < g_tipPos.size(); i++) {
		// CONVERT P FROM BUNNY COORDS TO WORLD COORDS
