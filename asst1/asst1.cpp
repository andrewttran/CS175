...
static float g_shiftX          = 0.0;       // how much to move the triangle horizontally from center
static float g_shiftY          = 0.0;       // how much to move the triangle vertically from center
...
struct SquareShaderState {
  ...
  GLint h_uWidthScale, h_uHeightScale;
...
static void drawSquare() {
  ...
  if (g_height < g_width) {
	  safe_glUniform1f(g_squareShaderState->h_uWidthScale, double(g_height)/g_width);
	  safe_glUniform1f(g_squareShaderState->h_uHeightScale, 1);
  }
  else {
	  safe_glUniform1f(g_squareShaderState->h_uWidthScale, 1);
	  safe_glUniform1f(g_squareShaderState->h_uHeightScale, double(g_width)/g_height);
  }
...
static void drawTriangle() {
  ...
  if (g_height < g_width) {
	  safe_glUniform1f(g_triangleShaderState->h_uWidthScale, double(g_height)/g_width);
	  safe_glUniform1f(g_triangleShaderState->h_uHeightScale, 1);
  }
  else {
	  safe_glUniform1f(g_triangleShaderState->h_uWidthScale, 1);
	  safe_glUniform1f(g_triangleShaderState->h_uHeightScale, double(g_width)/g_height);
  }
  safe_glUniform1f(g_triangleShaderState->h_uShiftX, g_shiftX); // shift triangle on X axis
  safe_glUniform1f(g_triangleShaderState->h_uShiftY, g_shiftY); // shift triangle on Y axis
...
static void loadTriangleShader(TriangleShaderState& ss) {
  ...
  ss.h_uWidthScale = safe_glGetUniformLocation(h, "uWidthScale");
  ss.h_uHeightScale = safe_glGetUniformLocation(h, "uHeightScale");
  ss.h_uShiftX = safe_glGetUniformLocation(h, "uShiftX");
  ss.h_uShiftY = safe_glGetUniformLocation(h, "uShiftY");
