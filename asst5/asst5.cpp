...
bool interpolateAndDisplay(float t) {
	if (floor(t) == g_keyFrames.size() - 1) { return true;} // check if end reached
	list<vector<RigTForm> >::iterator i = g_keyFrames.begin();
	advance(i, floor(t));
	vector<RigTForm> frame1 = *i; // floor(t) element
	vector<RigTForm> frame2 = *(++i); // floor(t) + 1 element
